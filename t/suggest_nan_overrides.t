...
like( $output, qr{previous=masterpoint_20260707_0130_3hcadence[.]txt}, 'previous source is reported' );
like( $output, qr{next=masterpoint_20260707_0730_3hcadence[.]txt}, 'next source is reported' );
like( $output, qr{UNRESOLVED\s+masterpoint_20260710_0430_3hcadence[.]txt\s+335.*reason=anchor_gap},
  'suggestions do not interpolate across distant anchors by default' );
ok( !-e $out, 'default scan is read-only' );

$output = qx("$^X" "$repo/suggest_nan_overrides.pl" -srcdir="$src" -wavelength=335 -outdir="$out" 2>&1);
...
like( $content, qr{^# target: masterpoint_20260707_0430_3hcadence[.]txt$}m, 'override records its target' );
like( $content, qr{^# previous: masterpoint_20260707_0130_3hcadence[.]txt$}m,
  'override records its previous source' );
like( $content, qr{^# next: masterpoint_20260707_0730_3hcadence[.]txt$}m,
...
$output = qx("$^X" "$repo/suggest_nan_overrides.pl" -srcdir="$src" -wavelength=335 -fits-root="$fits" -outdir="$limb_out" -radius-tolerance=1 -neighbor-tolerance=0.25 -agreement-tolerance=0.25 -min-limb-samples=10 2>&1);
...
like( $limb_content, qr{^# method: limb validated by temporal interpolation$}m,
...
$output = qx("$^X" "$repo/suggest_nan_overrides.pl" -srcdir="$src" -wavelength=335 -fits-root="$fits" -outdir="$conflict_out" -radius-tolerance=1 -neighbor-tolerance=0.25 -agreement-tolerance=0.25 -min-limb-samples=10 2>&1);
is( $? >> 8, 0, 'limb-aware conflict scan exits successfully' ) or diag $output;
like( $output, qr{CONFLICT\s+masterpoint_20260707_0430_3hcadence[.]txt\s+335},
  'disagreeing limb and temporal evidence is reported as a conflict' );
ok( !-e "$conflict_out/masterpoint_20260707_0430_3hcadence.overrides.txt",
  'conflict does not write an override file' );

write_limb( "$fits/2026/07/07/20260707_03_0335.limb", 2041, 2046.5, 0 );
write_limb( "$fits/2026/07/07/20260707_00_0335.limb", 100, 100, 0 );
$output = qx("$^X" "$repo/suggest_nan_overrides.pl" -srcdir="$src" -wavelength=335 -fits-root="$fits" -radius-tolerance=1 -neighbor-tolerance=0.25 -agreement-tolerance=0.25 -min-limb-samples=10 2>&1);
is( $? >> 8, 0, 'biased-anchor scan exits successfully' ) or diag $output;
like( $output, qr{UNRESOLVED\s+masterpoint_20260707_0430_3hcadence[.]txt\s+335\s+previous=NONE},
