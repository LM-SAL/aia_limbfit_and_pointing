...
like( $output, qr/No valid \.limb files reduced/, 'empty reducer run reports missing inputs' );
ok( !-e $stale_out, 'empty reducer run removes stale output file' );
ok( !-e $stale_stage, 'empty reducer run removes stale stage file' );
...
like( $output, qr/Can't stage/, 'stage copy failure is reported' );
